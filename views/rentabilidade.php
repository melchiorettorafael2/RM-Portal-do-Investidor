        ...
        <?php
         ...
         ?> 
          ...
          <?php while($dado = $con->fetch_array()){
             ...
             ?>
             <?php } ?>
            <a href="<?php echo $link;?>" id="active" title="Rentabilidade">&nbsp;<i class="fas fa-funnel-dollar"></i><span>Rentabilidade</span></a>
    ...
    <div class="container">
      <div class="card" id="resgate">
        <div class="card-header">
           <?php if ($language ==='en_usa') { ?> 
                <h3><i class="fas fa-funnel-dollar"></i>&nbsp;&nbsp;Profitability Decision</h3>
            <?php } else { ?>
                <h3><i class="fas fa-funnel-dollar"></i>&nbsp;&nbsp;Decisão de Rentabilidade</h3>
            <?php } ?>
        </div>
        <div class="form group container-fluid">
                     <form method="POST" id="form" class="needs-validation" action="mail_rentabilidade.php" novalidate>
                      <div class="form row mt-3">
                        ...
                        <?php
                          if ($language ==='en_usa') {
                              ...
                              $saldo =  $finance->rentabilidade;
                              $montante = $finance->montante;
                              $rentabilidade = $finance->rentabilidade;
                              $local = 'pt-br';
                              $currency = 'BRL';
                              $timezone = new DateTimeZone('America/Sao_Paulo');
                              $mes_atual = date('m');
                              $juros = ($montante + $rentabilidade) * 0.02;
                              $consulta_saldo_rent = "SELECT * FROM rentabilidade where id_user=$uid AND MONTH(data)=$mes_atual AND status='Pago'";
                              $con_saldo = $conexao->query($consulta_saldo_rent);
                              $contador_rows = $conexao->query($consulta_saldo_rent);
                              
                              while($valor= $con_saldo->fetch_array()){
                                  $valor_rent = $valor['valor'];
                                  
                              }
                              
         
                               $num_linhas = mysqli_num_rows($contador_rows);
                                  
                                if($num_linhas>0 && $juros>1){
                                    $juros = ((($montante + $rentabilidade - $valor_rent)) *0.02) - $valor_rent;                                      
                                      
                                }else if($num_linhas>0 && $juros<1){
                                    $juros=0;
                                }else{
                                      $juros = ($montante + $rentabilidade) * 0.02;
                                  }
                              ...
                              $fmt = new NumberFormatter($local, NumberFormatter::CURRENCY);
                              $padrao_br = $fmt->formatCurrency($saldo, $currency);
                              $juros_br = $fmt->formatCurrency($juros, $currency);
                        ...
                        <div class="form-group col-md-6">
                          <?php if ($language ==='en_usa') { ?> 
                               <label>Available Profitability Balance:</label>
                          <?php } else { ?>
                                <label>Saldo Disponível de Rentabilidade:</label>
          ...
          <?php } ?>
                           ...
                           <div class="input-group-prepend">
                              ...
                              <input type="text" readonly class="form-control bg-white text-dark" id="input_saldo" name="inputEmail"value="<?php echo $juros_br;?>">
                           </div>
                        </div>
                       </div>
                       ...
                       <div class="form-group">
                            <?php if ($language ==='en_usa') { ?> 
                                <label for="exampleFormControlSelect1">Profitability Type</label>
                            <?php } else { ?>    
                               <label for="exampleFormControlSelect1">Tipo de Rentabilidade</label>
                             <?php } ?>
                             ...
                             <select class="form-control" name="resgate" id="selectOption">
                                <?php if ($language ==='en_usa') { ?> 
                                    ...
                                    <option selected>Monthly Profitability Rescue</option>
                                    
                                 <?php } else { ?>
                                       <option>Reinvestir</option>
                                       <option selected>Resgate de Rentabilidade Mensal</option>
                                 <?php } ?>
                          ...
                          <?php if ($language ==='en_usa') { ?> 
                               <script>
                                  ...
                                  if(option=="Monthly Profitability Rescue"){
                                    var opcao = "Monthly Profitability Rescue"
                                    window.location.href = "resgate_rentabilidade_mensal.php?language=en_usa";
                                  }else if(option=="Partial Rescue"){
                                    window.location.href = "resgate_parcial.php?language=en_usa";
                                  }else if(option=="Reinvest"){
                                      window.location.href = "rentabilidade.php?language=en_usa";
                                  }
                                  });
                            ...
                            <script>
                                  ...
                                  }else if(option=="Resgate Parcial"){
                                    window.location.href = "resgate_parcial.php";
                                  }else if(option=="Reinvestir"){
                                      window.location.href = "rentabilidade.php";
                                  }
                                  });
                       ...
                       <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            <?php if ($language ==='en_usa') { ?> 
                             ...
                             <?php } else { ?>
                                 <label>Período Liberado para Alteração:</label>
                             <?php } ?>
                           <div class="input-group-prepend">
            ...
            <div class="card-body">
                <?php if ($language ==='en_usa') { ?>
                    <p class="h6">For this request, it will not be possible to change bank details without prior notice of 5 days prior to the scheduled date.</p>
                    ...
                    <p class="h6 text-danger">*All transfers will be made during business hours.</p>
                <?php } else { ?> 
                    <p class="h6">Para esta solicitação não será possível realizar a alteração dos dados bancários sem o aviso prévio de 5 dias de antecedência da data agendada.</p>
                    <p class="h6">Se a alteração for informada com o prazo inferior a 5 dias, você deverá optar por reinvestir na conta já cadastrada, ou cancelar a solicitação, efetuando a abertura de uma nova solicitação com os dados atualizados.</p>
                    <p>Caso haja necessidade de alteração dos dados da conta bancária, será necessário a abertura de um chamado no portal do cliente.</p>
                    <p class="h6 textr">Todas as transferências serão feitas no horário comercial (das 09:00 às 17:00).</p>
                <?php } ?>
                            ...
                            if(saldo>=valorConvert2){
                                  if(valor!=""){
                                      swal({
                                        ...
                                        text: "Would you like to confirm the reinvestment in the amount of $ " +valor+ " on the date of " +data+ " to your Klaus Invest fund account?",
                                        icon: "warning",
                                    ...
                                    .then((willDelete) => {
                                        if (willDelete) {
                                            swal("Successfully Requested Reinvestment!", "",{
                                            icon: "success",
                                            button: "OK",
                                            dangerMode: true,
                                });
                               ...
                               swal({
                                  ...
                                  dangerMode: true,
                                        ...
                                        swal({
                                          title: "Enter an amount to reinvest",
                                          text: "",
                                          icon: "warning",
                                          button: "OK",
                                          dangerMode: true,
         ...
         <?php } else { ?>
                  <script>
                     function reinvestir(){
                         var saldo = <?php echo $finance->rentabilidade?>;
                        var montante = <?php echo $finance->montante?>;
                        var juros = (montante + saldo + 0.01) * 0.02; 
                        var valor =  document.getElementById('input_valor').value;
                        var valorConvert = valor.replace(".","");
                        var valorConvert2 = valorConvert.replace(",",".");
                        var data = document.getElementById('input_data').value;
                            ...
                            if(valorConvert2<=juros){
                                  if(valor!=""){
                                      swal({
                                        title: "Confirmar Rentabilidade Mensal?",
                                        text: "Você confirma a solicitação de rentabilidade mensal no valor R$ " +valor+ " na data de " +data+ " para a sua conta do fundo Klaus Invest?",
                                        icon: "warning",
                                    ...
                                    .then((willDelete) => {
                                        if (willDelete) {
                                            swal("Rentabilidade Mensal solicitada com Sucesso!", "",{
                                            icon: "success",
                                            button: "OK",
                                            dangerMode: true,
                                });
                               ...
                               swal({
                                  title: "Solicitação de Rentabilidade Cancelada!",
                                  text: "",
                                  icon: "error",
                                  button: "OK",
                                  dangerMode: true,
                                        ...
                                        swal({
                                          title: "Informe um valor para a sua rentabilidade mensal",
                                          text: "",
                                          icon: "warning",
                                          button: "OK",
                                          dangerMode: true,
                                ...
                                swal({
                                  title: "Erro ao Solicitar Rentabilidade!",
                                  text: "Saldo de Rentabilidade Insuficiente para esta operação.",
                                  icon: "error",
                                  button: "OK",
                                  dangerMode: true,
                                })
        ...
        <?php if ($language ==='en_usa') { ?>
            <center><input type="button" onclick="reinvestir();" class="btn btn-info mt-2 mb-3" value="Solicit"></center>
        <?php } else { ?>
        <center><input type="button" onclick="reinvestir();" class="btn btn-info mt-2 mb-3" value="Solicitar"></center>
        <?php } ?>
...
<!--Permissão de Alteração de Plano-->
  <script>
			...
			window.onload = function disable_select(){	
