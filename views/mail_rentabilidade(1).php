...
@$tipo_resgate = $_POST['resgate'];
@$status= 'Em Análise';
@$data_pagamento='Análise';
@$tipo="Rentabilidade";
 $db = getDB();
       $stmt = $db->prepare("INSERT INTO rentabilidade(tipo, data, investidor, id_user, valor,status)  VALUES (:tipo,:data,:investidor,:id_user,:valor,:status)");
...
$mail = new PHPMailer\PHPMailer();
  ...
  $mail->Subject=("Solicitação de Reinvestimento.");
...
<head>
   ...
   <table border="0" cellpadding="0" cellspacing="0" width="100%">
     ...
     <tr>
      <td style="padding: 20px 0 30px 0; color: #153643; font-family: Arial, sans-serif; font-size: 16px; line-height: 20px;">
          Eu,&nbsp;'.@$name.',<br>gostaria de mudar minha opção para resgate de rentabilidade mensal no valor de R$ '.@$valor.' na data:&nbsp;'.@$data.' no fundo de investimento Klaus Invest.
      </td>
     </tr>
     <tr>
      <td>
 ...
 <tr>
  ...
  if($mail->send()){
    header("refresh: 0;resgate_rentabilidade_mensal.php");
   
  } else{
    echo "<script>alert('Erro ao solicitar Rentabilidade, tente diretamente pelo e-mail contato@klausinvest.com');</script>";
     header("refresh: 0;resgate_rentabilidade_mensal.php");
